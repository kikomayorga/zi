...
then
	...
	cd /etc/zi && lua zi.lua key 8 $lkchain
...
then 
	...
	killall -q aplay && killall -q lua && echo 0000 > /tmp/zi/last4keys && echo 0 > /tmp/zi/busyflag && echo 0 > /tmp/zi/skippableflag && cd /etc/zi && lua zi.lua key 8 0000
fi
